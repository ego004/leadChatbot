...
class ClientUser(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint('client_id', 'phone_number', name='uq_client_user_phone_per_client'),
    )

    user_id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    client_id = Column(String(36), ForeignKey("clients.client_id"), nullable=False)
    email = Column(String(255), nullable=False)
    phone_number = Column(String(20), nullable=True)  # Optional: allow email-only accounts
